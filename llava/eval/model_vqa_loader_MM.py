...
def get_disjoint_segments(attn_layers, P,begin_pos_vis_att, vis_len = 576, return_single = False , insert_mask = None):
    ...
    if insert_mask is None:
        ...
        combo, ent = combine_heads(attn_last_to_vis, selected[: 3], P=P, sigma=None)
        
        combo2 = gaussian_filter(combo, sigma=2.0)
        mask_grid = binarize_mean_relu(combo2, ent)
        if DO_PLOT:
...
def get_indices_percent(attn_layers, begin_pos_vis_att, vis_len = 576, mode = "selected", topK = 0.9, largest = False, sample = False,attn_mean_all = None, general_att_map = None, width = 1, height = 1):
    ...
    if mode == "topK":
        ...
        selected = selected[:3]
        combo, ent = combine_heads(attn_last_to_vis, selected[: 3], P=24, sigma=2.0)
...
def rel_attention_llava(image, att_map, general_prompt, model, processor):
    ...
    general_att_map = model(**general_inputs, output_attentions=True)['attentions'][14][0, :, -1, general_pos:general_pos+576].mean(dim=0).to(torch.float32).detach().cpu().numpy().reshape(NUM_PATCHES, NUM_PATCHES)
...
def eval_model(args):
    ...
    for (input_ids, image_tensor, image_sizes), line in tqdm(zip(data_loader, questions), total=len(questions)):
        ...
        with torch.inference_mode():
            ...
            attn , first_hidden_orig = get_attn_layers(model,input_ids,image_tensor, image_sizes[0], input_embeds = orig_embeds, num_layer = 25, attention_mask = att_mask,position_ids = position_ids )
      
            attn = attn[14].mean(0)[0][begin_pos_vis:begin_pos_vis + vis_len].reshape((24,24)).cpu().numpy()

            x_min,y_min, x_max, y_max = bbox_from_att_image_adaptive(attn, image_sizes[0], begin_pos_vis, bbox_size=336, vis_len = 576 )
            bbox =  [x_min,y_min, x_max, y_max    ]
            img_cropped = get_image(dataset, index, box = None, P = 24)
            img_cropped = img_cropped.crop(bbox)
            image_tensor = process_images([img_cropped], dataset.image_processor, dataset.model_config)[0]
            if image_tensor.shape[0] != 1:
                image_tensor = image_tensor[0,:,:,:].unsqueeze(0)
