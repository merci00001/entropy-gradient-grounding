...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index, bbox = None , P = PATCH, split = False, n = 4, path = None, augmentation = None):
...
def analyze_heads(attn: torch.Tensor):
    ...
    """
    L, H, _, V = attn.shape
    P = PATCH #int(meta.get("patch_size", int(np.sqrt(V))))
...
def get_disjoint_segments(attn_layers, P,begin_pos_vis_att, vis_len = 576, return_single = False , insert_mask = None, grad = None):
    ...
    if insert_mask is None and grad is None:
        ...
        combo, ent = combine_heads(attn_last_to_vis, selected[:10 ], P=P, sigma=2.0)
        print(ent)
        ...
        grad_orig = grad.reshape(PATCH,PATCH)#.detach().cpu().to(torch.float32).numpy()
        combo = grad_orig
...
def get_indices_percent(attn_layers, begin_pos_vis_att, vis_len = 576, mode = "selected", topK = 0.9, largest = False, sample = False,attn_mean_all = None, general_att_map = None, width = 1, height = 1,grad = None):
    ...
    if mode == "topK":
        ...
        combo, ent = combine_heads(attn_last_to_vis, selected[: 3], P=PATCH, sigma=None)
        ...
        attn_pic = attn_layers[14,:,0,begin_pos_vis_att:begin_pos_vis_att + vis_len].mean(0).reshape(PATCH,PATCH) / general_att_map
...
def eval_model(args):
    ...
    for (pixel_values, input_ids, attention_mask), line in tqdm(zip(data_loader, questions), total=len(questions)):
        ...
        input_ids_normal = torch.cat((input_ids[:begin_pos_vis],input_ids[begin_pos_vis : begin_pos_vis + vis_len] , input_ids[end_pos_vis :] ))
        input_ids_full = torch.cat((input_ids[:begin_pos_vis],input_ids[begin_pos_vis : begin_pos_vis + vis_len] ,input_ids[begin_pos_vis : begin_pos_vis + vis_len] , input_ids[end_pos_vis :] ))
        img_id = line["image"]
        ...
        if True:#with torch.inference_mode():
            ...
            orig_embeds = get_embedding(model,input_ids_normal,pixel_values[-1])
            ...
            if True:
                with torch.no_grad():
                    generated_ids = model.generate(input_ids = input_ids_normal.unsqueeze(0) , inputs_embeds = orig_embeds, max_new_tokens=200,
                                                    do_sample=True if args.temperature > 0 else False,
                                                    temperature=args.temperature,
                                                    top_p=args.top_p,
                                                    num_beams=args.num_beams,)
                
                input_len = len(input_ids_normal)
            ...
            if METHOD == "attention":
                attn_orig , first_hidden_orig = get_attn_layers(model,input_ids_normal,None, None, input_embeds = orig_embeds, num_layer = None, attention_mask = None,position_ids = None )
            else:
                attn_orig = None

            boxes, sorted_order = get_disjoint_segments(attn_orig, PATCH,begin_pos_vis, vis_len = vis_len, return_single = False ,insert_mask = None, grad = grad_orig_flat)
            ...
            for ind, m in enumerate(boxes):
                ...
                pixel_values2, input_ids2, attention_mask2 = dataset.__getitem__(index  , bbox = box , P = PATCH, split = False, n = 4, path = None, augmentation = None)  #f"{output_dir}/cut.png"

                pixel_values2 = pixel_values2.to("cuda")
                input_ids2 = input_ids2.to("cuda")
                attention_mask2 = attention_mask2.to("cuda")

                indices_orig =torch.tensor( box_to_indices(box, PATCH)  )
                
                flattened = torch.flatten(torch.tensor(m))
                
                #indices_orig= ord #torch.where(flattened)[0]

                emb_vis = get_embedding(model,input_ids_normal,pixel_values2[-1])
             
                prune_type = "topK"

                if prune_type != "grad":
                    attn, logits  = get_attn_layers(model,input_ids_normal,None, None, input_embeds = emb_vis, num_layer = None, attention_mask = None,position_ids = None )
                    grad = None
                else:
                    attn = None
                ...
                if DO_PLOT:
                    im = get_image(dataset, index, box = box, P = PATCH)
                ...
                indices, ent = get_indices_percent(attn, begin_pos_vis, vis_len = vis_len, mode = prune_type, topK = len(indices_orig) / (PATCH*PATCH), largest = True, width = box[2] , height =box[3] , sample = False,grad = grad ) #bsize / (24*24)
            ...
            if DO_PRUNE: 
                ...
                indices, ent = get_indices_percent(None, begin_pos_vis, vis_len = 576, mode = "selected", topK = 5/(PATCH*PATCH), largest = True , grad =grad )
            ...
            with torch.no_grad():
                generated_ids = model.generate(input_ids = input_ids_full.unsqueeze(0) , inputs_embeds = orig_embeds, max_new_tokens=200,
                                                do_sample=True if args.temperature > 0 else False,
                                                temperature=args.temperature,
                                                top_p=args.top_p,
                                                num_beams=args.num_beams,)
