...
class CustomDataset(Dataset):
    def __init__(self, questions, image_folder, model, processor,tokenizer, model_config):
    ...
    def __getitem__(self, index, bbox = None , P = (24,24), split = False, n = 4, path = None, augmentation = None, insert_image = None):
        ...
        return input_ids,image_grid_thw, pixel_values, image
...
def collate_fn(batch):
    input_ids,image_grid_thw, pixel_values, image = zip(*batch)
...
# DataLoader
def create_data_loader(questions, image_folder, model, processor,tokenizer, model_config, batch_size=1, num_workers=4):
    assert batch_size == 1, "batch_size must be 1"
    dataset = CustomDataset(questions, image_folder, model, processor,tokenizer, model_config)
...
def get_disjoint_segments(attn_layers, P,begin_pos_vis_att, vis_len = 576, return_single = False , insert_mask = None, grad = None):
    ...
    if insert_mask is None and grad is None:
        ...
        grad_orig =  gaussian_filter(grad_orig, sigma=2.5)
        grad_orig2 = grad_orig.copy()
        
        el =grad_orig.max() * 0.5  #   elbow_chord(grad_orig_filtered.flatten())
...
def calc_grad(model, input_ids, input_embeds, attention_mask=None):
    model.requires_grad_(False)
    ...
    with torch.set_grad_enabled(True):
        ...
        objective = torch.sum(probs * torch.log(safe_probs))

        print("Entropy: ", objective * -1)
        objective.backward()
...
def eval_model(args):
    ...
    data_loader,dataset = create_data_loader(questions, args.image_folder, model, processor,tokenizer, model.config, batch_size = 1)
    ...
    for (input_ids,image_grid_thw, pixel_values, image), line in tqdm(zip(data_loader, questions), total=len(questions)):


        pixel_values = pixel_values[0].to("cuda")
        input_ids = input_ids[0].to("cuda")
        image_grid_thw = image_grid_thw[0].to("cpu")
        print("grid: ", image_grid_thw)
        ratio = image_grid_thw[0][1] /  image_grid_thw[0][2]
        img_id = line["image"]
        ...
        begin_pos_vis = (input_ids == 151652).nonzero(as_tuple=True)[1][0].item() + 1
        ...
        if True:#with torch.inference_mode():
            ...
            orig_embeds , vis_len= get_embedding(model,input_ids,pixel_values,image_grid_thw)
            measure_attention_vram_forward(model, orig_embeds, attention_mask=None)
            return
            if DO_VANILLA or DO_PLOT:
          
                seq_len = orig_embeds.shape[1]
                attention_mask = torch.ones(1, seq_len, dtype=torch.long, device=orig_embeds.device)
                ...
                text2 = processor.batch_decode(outputs.sequences, skip_special_tokens=True)[0]
                if DO_PLOT:
                    orig_answer = text2
                    print("original Answer: ", text2)
                else:
                    print(img_id)
            ...
            print("vis len: " , vis_len)
            
            token_width = int(np.sqrt(vis_len*ratio))
            token_height = int(vis_len / token_width)

            tokens_total = [(token_width,token_height)]
            ...
            crops = []
            if METHOD == "grad":
                g = calc_grad(model,input_ids, orig_embeds, attention_mask=None)[
                ...
                if DO_PLOT:
                    ...
                    #grad_orig_flat2 = F.softmax(grad_orig_flat2 / temperature, dim=-1)
                    grad_orig = grad_orig_flat2.reshape(token_width,token_height).detach().cpu().to(torch.float32).numpy()
                    ...
                    #grad_orig_filtered[grad_orig > threshold] = gaussian_filter(grad_orig[grad_orig > threshold], sigma=1.5)
                    grad_mask = grad_orig_filtered > grad_orig.max()* 0.2  #binarize_mean_relu(grad_orig, do_max = True)
                    ...
                    grad_reshaped = grad_orig.reshape(token_width,token_height)
                    ...
                    plot_mask(im, indices_grad, vis_len, f"GradMask", question,token_width,token_height  )

                    tensor_np = grad_reshaped #.detach().cpu().numpy()

                    # normalize for visualization (important for gradients)
                    tensor_np = (tensor_np - tensor_np.min()) / (tensor_np.max() - tensor_np.min() + 1e-8)

                    plt.imsave(
                        "/cluster/scratch/mgroepl/debug/grad.png",
                        tensor_np,
                        cmap="viridis"
                    )
            ...
            while True:
                ...
                images_new = [image]
                pointer = 0
                for ind, (w,h) in enumerate(tokens_total):
                    b, sorted_order, ent = get_disjoint_segments(attn_orig, (w,h),begin_pos_vis, vis_len = vis_len, return_single = False ,insert_mask = None, grad = grad_orig_flat[pointer: pointer + w*h])

                    if DO_PLOT:

                        grad_orig = grad_orig_flat[pointer: pointer + w*h].reshape(w, h).detach().cpu().float().numpy()

                        # Optional normalization (recommended for gradients)
                        grad_norm = (grad_orig - grad_orig.min()) / (grad_orig.max() - grad_orig.min() + 1e-8)

                        plt.imsave(
                            f"/cluster/scratch/mgroepl/debug/{Iteration}grad.png",
                            grad_orig,
                            cmap="viridis"
                        )
                ...
                try:
                    ...
                    boxes = boxes[: min(len(boxes),3)]
                except:
                    break
                box_index = [x for _, x in sorted(zip(vals_summed, box_index), reverse=True)]
                box_index = box_index[: min(len(box_index),3)]

                ents = [x for _, x in sorted(zip(vals_summed, ents), reverse=True)]
                ents = ents[: min(len(ents),3)]


                total_vis = [orig_embeds[0,:begin_pos_vis+vis_len]]
                ...
                for ind, m in enumerate(boxes):
                    ...
                    print("tokens_total", tokens_total, box_index)

                    input_ids2,image_grid_thw_2, pixel_values_2, image_2= dataset.__getitem__(index  , bbox = box , P = tokens_total[box_index[ind]], split = False, n = 4, path = None, insert_image = images[box_index[ind]])  #f"{output_dir}/cut.png"
                    images_new.append(image_2)
                   
                    input_ids2 = input_ids2.to(device='cuda', non_blocking=True)
                    orig_embeds2 , vis_len2 = get_embedding(model,input_ids2,pixel_values_2,image_grid_thw_2)
                    print("vis len: " , vis_len2)
                    image_grid_thw_2 = image_grid_thw_2[0]
                    print("image_grid_thw_2", image_grid_thw_2)
                    ratio = image_grid_thw_2[1] /  image_grid_thw_2[2]
                    token_width2 =  int(np.sqrt(vis_len2*ratio)) #int(image_grid_thw_2[0] / 2) #
                    token_height2 =   int(vis_len2 / token_width2)# int(image_grid_thw_2[1] / 2) #int(vis_len / token_width)
                    print("ratio", ratio,"token_width2", token_width2, "token_height2",token_height2 )
                    tokens_total_new.append((token_width2,token_height2))
                    input_ids2 = input_ids2.to(device='cuda', non_blocking=True)
                
                    begin_pos_vis2 = (input_ids2 == 151652).nonzero(as_tuple=True)[1][0].item() + 1
                    print("shape",orig_embeds2.shape)
                    print("begin_pos_vis2 ", begin_pos_vis2)
                    new_vis = orig_embeds2[0,begin_pos_vis2:begin_pos_vis2+ vis_len2]
                    total_vis.append(new_vis)
                    ...
                    continue
                images = images_new
                tokens_total = tokens_total_new
                total_vis.append(orig_embeds[0,begin_pos_vis+1+vis_len:,:])
                
                new_embeds = torch.cat(total_vis,dim = 0).unsqueeze(0)
                
                seq_len = new_embeds.shape[1]
                attention_mask = torch.ones(1, seq_len, dtype=torch.long, device=new_embeds.device)

                g = calc_grad(model,None, new_embeds, attention_mask=attention_mask)[
                        0, begin_pos_vis:, :
                    ] 
                
                print("new grad",g.shape)
                grad_orig_flat = torch.norm(
                    g,
                    p=2,           # e.g., p=1 (L1), p=2 (L2), p='inf'
                    dim=-1
            ...
            print("new_embeds",new_embeds.shape)        

            # inputs_embeds shape: [1, seq_len, hidden_size]
            seq_len = new_embeds.shape[1]
            attention_mask = torch.ones(1, seq_len, dtype=torch.long, device=new_embeds.device)
            ...
            if False:
                ...
                inputs = forward_with_images(model, processor, question, [crops[0]])

                with torch.no_grad():
                    generated_ids = model.generate(
                                    **inputs,
                                    do_sample=False,   # temperature=0 means greedy, so do_sample=False
                                    temperature=None,  # not needed when do_sample=False
                                    max_new_tokens=100,
                                )

                # Trim input tokens from output
                trimmed = [out[len(inp):] for out, inp in zip(generated_ids, inputs.input_ids)]
                text2 = processor.batch_decode(trimmed, skip_special_tokens=True)[0]
                print("text before ", text2)
                
                inputs = forward_with_images(model, processor, question, crops)
            ...
            if DO_PLOT:
                print("Answers",orig_answer,text2 )
