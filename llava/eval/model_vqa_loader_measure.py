...
class CustomDataset(Dataset):
    def __init__(self, questions,multi_image, image_folder, tokenizer, image_processor, model_config):
    ...
    def __getitem__(self, index, bbox = None , P = 24, split = False, n = 4, path = None, augmentation = None):
        ...
        print("self.multi_image" , self.multi_image)
...
# DataLoader
def create_data_loader(questions,multi_image, image_folder, tokenizer, image_processor, model_config, batch_size=1, num_workers=4):
    assert batch_size == 1, "batch_size must be 1"
    dataset = CustomDataset(questions, multi_image, image_folder, tokenizer, image_processor, model_config)
    ...
    with torch.inference_mode():
        ...
        if input_embeds is not None and num_layer is None:
            ...
            for i, layer in enumerate(encoder_layers[:num_layers_to_run]):
                ...
                flop_count = FlopCountAnalysis(layer, inputs)
                        
                number = flop_count.total()
            ...
            return attn_outputs, totalFlops
...
def eval_model(args):
    ...
    tokenizer, model, image_processor, context_len = load_pretrained_model(model_path, args.model_base, model_name, device = "cuda", use_flash_attn = False if METHOD == "grad" else False , attn_implementation="eager"
)   
    ...
    print("args.multi_image", args.multi_image)
    data_loader,dataset = create_data_loader(questions,args.multi_image, args.image_folder, tokenizer, image_processor, model.config, batch_size = 1)
    ...
    for (input_ids, image_tensor, image_sizes), line in tqdm(zip(data_loader, questions), total=len(questions)):
        ...
        if True:#with torch.inference_mode():
            ...
            memory_after_forward_pass = torch.cuda.memory_allocated()

            memory_used_GB = (memory_after_forward_pass - initial_memory) / (1024 ** 3)
            print(f"Memory used during forward pass: {memory_used_GB:.3f} GB")
            orig_embeds , att_mask, position_ids= get_embedding(model,input_ids,image_tensor,image_sizes)
            ...
            if METHOD == "attention":
                attn_orig , totalFlops = get_attn_layers(model,input_ids,image_tensor, image_sizes, input_embeds = orig_embeds, num_layer = 15, attention_mask = att_mask,position_ids = position_ids )
                

                flops_total += totalFlops 
            else:
                attn_orig = None
            ...
            for ind, m in enumerate(boxes):
                ...
                attn, totalFlops  = get_attn_layers(model,input_ids, image_tensor2.unsqueeze(0).to(dtype=torch.float16, device='cuda', non_blocking=True),None, input_embeds = emb_vis, num_layer = 15, attention_mask = att_mask, position_ids = position_ids)
            ...
            outputs = model.generate(
                None,
                attention_mask = None,
                inputs_embeds = orig_embeds if DO_PRUNE == False else orig_embeds_pruned,
            ...
            flops_total_mean.append(flops_total)
            number = flops_total
            formatted_number = f"{number:,}"
            print("flops_total" ,formatted_number)
            ...
            if DO_PLOT:
                return
        formatted_number = f"{sum(flops_total_mean) / len(flops_total_mean):,}"
  
        sequences = outputs.sequences

        print("Flops averaged over ", INDEX, " runs: ", formatted_number)   
        ...
        # ans_file.flush()
    formatted_number = f"{sum(flops_total_mean) / len(flops_total_mean):,}"
    print("Flops averaged over ", 5000, " runs: ", formatted_number)   
