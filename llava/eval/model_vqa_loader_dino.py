...
def get_general(tokenizer, model, image_tensor, image_sizes, begin_pos_vis,vis_len ):
    ...
    general_att_map, hidden   = get_attn_layers(model,input_ids_general,image_tensor, image_sizes, input_embeds = orig_embeds_general, num_layer = 15, attention_mask = att_mask_general,position_ids = position_ids_general )[14,:,0,begin_pos_vis:begin_pos_vis + vis_len].mean(0).reshape(24,24)
...
def eval_model(args):
    ...
    for (input_ids, image_tensor, image_sizes), line in tqdm(zip(data_loader, questions), total=len(questions)):
        ...
        with torch.inference_mode():
            ...
            first_hidden_orig = first_hidden_orig[0,-1,:]
            print(first_hidden_orig.shape)
            ...
            if False:
                ...
                for x in outputs.hidden_states:
                    ...
                    for y in x:
                        ...
                        if lnew > l:
                            l = lnew
                    current_head = torch.stack((current_head), dim = 0)
                    flat.append(current_head.unsqueeze(0))
                hidden_tensors = torch.cat((flat), dim = -2).squeeze(0).squeeze(1)
            
                # Now stack
                gen_len = hidden_tensors.shape[1] - orig_embeds.shape[1]  
                H1 = hidden_tensors[:,-gen_len,:]


            output_dir = f"/cluster/scratch/mgroepl/heatmaps/black/{img_id}"
            ...
            for m in boxes:
                ...
                image = Image.open(os.path.join(dataset.image_folder, line["image"])).convert('RGB')
                P = 24
                a,b,width,height = box
                ...
                boxes_dino, logits, phrases = predict(
                    model=model_dino,
                ...
                if len(boxes_dino) > 0:
                    ...
                    b_dino = boxes_dino[0]
                    b_dino[0] =int(b_dino[0] - b_dino[2]/2)
                    b_dino[1] = int(b_dino[1] - b_dino[3]/2)
                    b_dino[0] *= image.size[0]
                    b_dino[1] *= image.size[1]
                    b_dino[2] *= image.size[0]
                    b_dino[3] *= image.size[1]
                    image_more_cropped = image.crop(b_dino.numpy())
                    #image_more_cropped.save("/cluster/project/cvg/students/mgroepl/LLaVA/llava/eval/output.png") 
                    
                    image_tensor2 = process_images([image_more_cropped], dataset.image_processor, dataset.model_config)[0]
                    ...
                    emb_vis, att_mask, position_ids = get_embedding(model,input_ids,image_tensor2.unsqueeze(0).to(dtype=torch.float16, device='cuda', non_blocking=True),[image_more_cropped.size])
                ...
                first_hidden = first_hidden[0,-1,:]
                cos_sim = F.cosine_similarity(first_hidden, first_hidden_orig, dim=0)
            ...
            if False:
                ...
                sim_matrix = torch.matmul(h1_norm, h2_norm.T)  # (600, 600)
                dist_matrix = torch.cdist(h1_norm, h2_norm, p=2)


                avg_diag_sim = sim_matrix.diag().cpu().numpy()
                plt.figure(figsize=(8, 4))
                plt.plot(avg_diag_sim)
                plt.title(f'  bbox {0} Orig: {text}, cut: {text2}')
                plt.xlabel("Index")
                plt.ylabel("Value")

                # Save the plot to a file
                plt.savefig(f"{output_dir}/sim.png", dpi=300, bbox_inches='tight')
                plt.close()

                dist_matrix = dist_matrix.diag().cpu().numpy()
                plt.figure(figsize=(8, 4))
                ...
                # Save the plot to a file
                plt.savefig(f"{output_dir}/euc.png", dpi=300, bbox_inches='tight')
                plt.close()
            ...
            #imgs = load_images_from_folder("/cluster/scratch/mgroepl/heatmaps/mean")
            #final = subtract_images(imgs[1:-2], imgs[0])
            #final = gaussian_filter(final, sigma=1.0)
            #print(final.max(),final.min())
            #mask = final > final.mean()
            #mask_img = Image.fromarray(mask.astype(np.uint8) * 255) 
            #flat_mask = mask.flatten()  # or mask.ravel()
            #true_indices = np.nonzero(flat_mask)[0]
         
            #plot_mask(f"{idx}.jpg", true_indices, vis_len, f"{idx}", question)
            #img = Image.fromarray(final, mode='L') 
            #path = get_unique_filename("/cluster/scratch/mgroepl/heatmaps/final/f")
