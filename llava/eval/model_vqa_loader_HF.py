...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index, bbox = None , P = 24, split = False, n = 4, path = None, augmentation = None, is_gt = False):
        ...
        inputs = self.processor(text = qs, images = image, return_tensors="pt", padding=True).to("cuda", torch.bfloat16)
...
def load_model(name):
    if 'llava' in name: 
        ...
        model = LlavaForConditionalGeneration.from_pretrained(name, torch_dtype=torch.bfloat16, low_cpu_mem_usage=True, attn_implementation="eager", device_map="cuda" ).to("cuda")
        processor = AutoProcessor.from_pretrained(name)
    elif args.model == 'blip':
        model = InstructBlipForConditionalGeneration.from_pretrained(name, torch_dtype=torch.bfloat16, low_cpu_mem_usage=True).to(args.device)
...
def eval_model(args):
    ...
    data_loader,dataset = create_data_loader(questions, args.image_folder, processor, model.config, batch_size = 1)
    avg_pruning = 0
    index = -1
    index_data = -1
    ...
    for (inputs), line in tqdm(zip(data_loader, questions), total=len(questions)):
        ...
        if box_dataset_orig[2] * box_dataset_orig[3] == 0:
            continue
        

        print(box_dataset_orig)
        box_dataset = [0,0,0,0]
        w = imgid_to_width[idx]
        h = imgid_to_height[idx]
        
        print(   (box_dataset_orig[2] * box_dataset_orig[3])/ (w*h)   )
        
        P_data_w = w //24 
        P_data_h = h // 24 
        box_dataset[0] = int(box_dataset_orig[0] //P_data_w) 
        box_dataset[1] = int(box_dataset_orig[1] // P_data_h) 
        box_dataset[2] =min( int(box_dataset_orig[2] // P_data_w) + 2 , 24)
        box_dataset[3] = min( int(box_dataset_orig[3] // P_data_h)  + 2 , 24)
        ...
        with torch.inference_mode():
            ...
            for ind, m in enumerate(boxes):
                ...
                if False:# str(ordered_id) in id_to_index:
                    box = box_dataset
                else:
                    box = bbox_from_mask(m)
                    if boxes_overlap(box, box_dataset) == False:
                        continue
                print(box)
                ...
                if str(ordered_id) in id_to_index:
                    _, image_tensor2, image_sizes2 = dataset.__getitem__(index  , bbox = box_dataset_orig , P = 24, split = False, n = 4, path = None, augmentation = None, is_gt = True)  #f"{output_dir}/cut.png"
                else:
                ...
                if bsize <0.0:
                    ...
                    emb_vis, att_mask, position_ids = get_embedding(model,input_ids_cropped,image_tensor2.unsqueeze(0).to(dtype=torch.float16, device='cuda', non_blocking=True),[image_sizes2])
                ...
                attn  = get_attn_layers(model,input_ids, image_tensor2.unsqueeze(0).to(dtype=torch.float16, device='cuda', non_blocking=True),None, input_embeds = emb_vis, num_layer = 15, attention_mask = att_mask, position_ids = position_ids).cuda()
                ...
                if DO_PLOT:
                    ...
                    if str(ordered_id) in id_to_index:
                        im = get_image(dataset, index, box = box_dataset, P = 24)
                ...
                if DO_PLOT and str(ordered_id) in id_to_index:
